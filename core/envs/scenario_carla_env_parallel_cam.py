...
class ScenarioCarlaEnvCam(BaseDriveEnv):
    """
    ...
    def __init__(
        ...
        self.scenario = int((port-9000)%5+20)
        self.state = np.zeros((100,600,3),dtype=np.float32)
    ...
    def reset(self):
        ...
        rgb1 = self.get_observations()['rgb1']
        rgb2 = self.get_observations()['rgb2']
        rgb3 = self.get_observations()['rgb3']

        segmentations = np.concatenate([segmentation3,segmentation1,segmentation2,rgb3,rgb1,rgb2], axis=1)
    ...
    def compute_reward(self):
        ...
        """
        goal_reward = 0
        if self._is_success and self._simulator.end_distance<1:
            goal_reward=10
        #if self._is_success:
        ...
        total_reward = goal_reward + failure_reward + speed_reward
