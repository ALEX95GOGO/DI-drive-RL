...
class CarFollowing(gym.Env):
    def __init__(self, enabled_obs_number=2, vehicle_type = 'random',
        ...
        self.surface = None
        self.camera_output = np.zeros([int(720/2),int(720/2),3])
        self.camera_output_2 = np.zeros([int(720/2),int(720/2),3])
        self.camera_output_3 = np.zeros([int(720/2),int(720/2),3])
...
#            self._handbrake_idx = int(self._parser.get('G29 Racing Wheel', 'handbrake'))
    ...
    def reset(self):
        ...
        self.seman_camera_2.listen(lambda image: CarFollowing._parse_seman_image_2(weak_self, image))
        self.seman_camera_3.listen(lambda image: CarFollowing._parse_seman_image_3(weak_self, image))
...
#                self.control.brake = np.float(abs(action))*1 if action<0 else 0   
        ...
        if self.front_control.throttle>0.75:
            self.front_control.throttle = 0.75
        dis = ((self.back_vehicle.get_location().x - self.front_vehicle.get_location().x)**2+(self.back_vehicle.get_location().y-self.front_vehicle.get_location().y)**2)**(1/2)
        #print(v_ego)
        if dis > 60:
        ...
        reward = - collision*100 + punishment + v_ego/8
        # - 0.5*(v_ego<0.2)
