...
from core.envs import DriveEnvWrapper, ScenarioCarlaEnvHAGRL
...
def main(args, cfg, seed=0):
    ...
    checkpoint_folder = 'dos1_new'
    #checkpoint_folder = 'logs/n1/2024-07-18_11-19-42'
    ...
    carla_env = DriveEnvWrapper(ScenarioCarlaEnvHAGRL(cfg.env, cfg.policy, configs, args.host, args.port))
    carla_env.seed(seed)
    set_pkg_seed(seed)
    ...
    model = PPO.load(os.path.join(checkpoint_folder,chekpoint_list[ii][:-4]),deterministic=True)
    obs = env.reset(i)
    #obs = plt.imread('/projects/MAD3D/Zhuoli/truck_project/truck/pedestrian_seg_new/segmented_frame_9673.png')
    #obs = torch.from_numpy(obs).cuda()
    #obs = torch.cat((obs[:,:,0:1],obs[:,:,0:1],obs[:,:,0:1]),dim=2)
    #obs = obs.transpose(0,2)
    #obs = F.interpolate(obs.unsqueeze(0), size=(100, 300), mode='nearest').squeeze()
    #obs = obs.cpu().numpy()
    #import pdb; pdb.set_trace()
    #env.render()
    #env.close()
    ...
    for k in range(9800,11500):
        # Load the model from the checkpoint
        #model = PPO("CnnPolicy", env, verbose=1, policy_kwargs=policy_kwargs, tensorboard_log="./ppo_tensorboard/")
        
        #model = PPO.load(chekpoint_list[ii])
        obs = plt.imread('/projects/MAD3D/Zhuoli/truck_project/truck/pedestrian_new_oneformer/seg_{}.png'.format(k))
        obs = torch.from_numpy(obs).cuda()
        obs[(obs[:,:,1]*255).int()==220]=0.5
        obs = torch.cat((obs[:,:,0:1],obs[:,:,0:1],obs[:,:,0:1]),dim=2)
        obs = obs.transpose(0,2)
        ...
        for i in range(obs.shape[0]):
            if torch.sum((obs[i]*255).int()==220)<50:    
                #self.human_map[(resize_observations[:,1:2]*255).int()==220]=1
                obs[i][obs[i]!=0]=0.5
            else:
                obs[i][(obs[i]*255).int()!=220]=0.5
                obs[i][(obs[i]*255).int()==220]=0
                #obs = 1-obs
        ''' 
        obs = F.interpolate(obs.unsqueeze(0), size=(100, 300), mode='nearest').squeeze()
        obs = obs.cpu().numpy()
        ...
        actions, _ = model.predict(obs*255)
        #obs, rewards, dones, infos = env.step(actions)
        ...
        upsampled_att = cv2.resize(machine_att, (1920, 780), interpolation=cv2.INTER_CUBIC)
        upsampled_att = np.squeeze(upsampled_att)
        scene_image = plt.imread('/projects/MAD3D/Zhuoli/truck_project/truck/Pedestrian_crash_new/frame_{}.png'.format(k)) 
        rgb_image = scene_image
        ...
        upsampled_att = upsampled_att[:,:]
        #import pdb; pdb.set_trace()
        plt.imshow(upsampled_att, cmap='hot', alpha=0.5)  # Alpha controls the transparency
        plt.axis('off')
        #plt.imshow(upsampled_att)
        #plt.show(block=False)
        #plt.pause(0.001)
        plt.savefig('/projects/MAD3D/Zhuoli/truck_project/truck/no_human_guided_pedestrian/frame_{}.png'.format(k), bbox_inches='tight', pad_inches=0)
        '''
        #env.render()
        #env.close()
        #break
