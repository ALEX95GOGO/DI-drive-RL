...
from core.envs import DriveEnvWrapper, ScenarioCarlaEnv_fixed_n4
...
def main(args, cfg, seed=0):
    ...
    chekpoint_list = []
    file_list_all = os.listdir('/projects/MAD3D/nips2024/DI-drive/demo/auto_run/{}'.format(checkpoint_folder))
    ...
    carla_env = DriveEnvWrapper(ScenarioCarlaEnv_fixed_n4(cfg.env, cfg.policy, configs, args.host, args.port))
    carla_env.seed(seed)
    set_pkg_seed(seed)
    ...
    for k in range(total_timesteps):
        ...
        if k>100:
            ...
            blend_path = '{}/blend_human'.format(image_folder)
            # Optionally, save the blended image to a file
            ...
            blend_path = '{}/blend'.format(image_folder)
            # Optionally, save the blended image to a file
            #image_path = os.path.join(blend_path, image_name)
            #image_path = os.path.join(blend_path, chekpoint_list[ii],'{}.png'.format(k))
            os.makedirs(blend_path, exist_ok=True)
