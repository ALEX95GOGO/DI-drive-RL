...
class CustomCNN(BaseFeaturesExtractor):
    ...
    def forward(self, observations: torch.Tensor) -> torch.Tensor:
        for i in range(observations.shape[0]):
                if torch.sum((observations[i]*255).int()==220)<1:
                    #self.human_map[(resize_observations[:,1:2]*255).int()==220]=1
                    observations[i][observations[i]!=0]=0.5
                else:
                    observations[i][(observations[i]*255).int()!=220]=0.5
        ...
        with torch.no_grad():
            ...
            #plt.imshow(self.observation[0,0].cpu())
            #plt.show(block=False)
            #plt.pause(0.01)
            #for i in range(rgb_image.shape[0]):
            #    if torch.sum((rgb_image[i]*255).int()==220)<1:
            #        #self.human_map[(resize_observations[:,1:2]*255).int()==220]=1
            #        rgb_image[i][rgb_image[i]!=0]=0.5
            #    else:
            #        rgb_image[i][(rgb_image[i]*255).int()!=220]=0.5
            ...
            #import pdb; pdb.set_trace()
            resize_observations = F.interpolate(observations, size=new_size, mode='nearest')
            for i in range(resize_observations.shape[0]):   
                if torch.sum((resize_observations[i,1:2]*255).int()==220)<1:
                    #self.human_map[(resize_observations[:,1:2]*255).int()==220]=1
                    self.human_map[i][resize_observations[i,1:2]==0]=0.3
            self.human_map[(resize_observations[:,1:2]*255).int()==220]=1
            #import matplotlib.pyplot as plt
            #plt.subplot(2,1,1)
            #plt.imshow(self.human_map[2,0].cpu())
            #plt.subplot(2,1,2)
            #plt.imshow(rgb_image[2,0].cpu())
            #plt.show(block=False)
            #plt.pause(0.01)
...
def main(args, cfg, seed=6):
    ...
    log_dir = os.path.join("logs", scenario_file[:-18:-12], datetime.datetime.now().strftime("%Y-%m-%d_%H-%M-%S"))
    ...
    # Optionally, save the trained model
    file_name = "dos1_human_guided_carla_model_{}_{}".format(current_time,scenario_file[:-18:-12])
    model.save(file_name)
