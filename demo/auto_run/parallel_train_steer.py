...
from core.envs import DriveEnvWrapper, ScenarioCarlaEnv4
...
def main(args, cfg, seed=0):
    ...
    def make_env(port):
        def _init():
            ...
            #env = ScenarioCarlaEnv(cfg.env, configs[0], args.host, port)
            env = ScenarioCarlaEnv4(cfg.env, cfg.policy ,routes, scenario_file, single_route, args.host, port)
            env.seed(seed)
            return env
        return _init
    ...
    model.save("ppo_carla_model_steer{}".format(scenario_file[:-18:-12]))
    #import pdb; pdb.set_trace()
    # Train the model
