...
#        episode_rewards = self.locals.get('rewards', [])
#
#        # Assuming episode_rewards is a list of rewards for the current step
#        reward_sum = sum(episode_rewards) if isinstance(episode_rewards, list) else episode_rewards.sum()
#        self.total_reward += reward_sum  # Accumulate rewards
#
#        if 'dones' in self.locals:
#            dones = self.locals['dones']
#            if any(dones):  # Check if the episode ended
#                # Log the total accumulated reward
#                self.logger.record("episode/reward", self.total_reward)
#                # Check for best model saving
#                if self.total_reward > self.best_mean_reward:
#                    self.best_mean_reward = self.total_reward
#                    self.model.save(self.best_model_path)
#                    if self.verbose > 0:
#                        print(f"New best mean reward: {self.best_mean_reward:.2f} - model saved!")
#
#                # Reset reward accumulator at the end of an episode
#                self.total_reward = 0
#        return True
...
def main(args, cfg, seed=0):
    ...
    ports = [9000 + 2*i for i in range(20)]
    ...
    file_name = "ppo_carla_model_{}_mixed".format(current_time)
