...
from core.envs import DriveEnvWrapper, ScenarioCarlaEnv_1cam
...
def main(args, cfg, seed=0):
    ...
    def make_env(port):
        def _init():
            ...
            #env = ScenarioCarlaEnv(cfg.env, configs[0], args.host, port)
            env = ScenarioCarlaEnv_1cam(cfg.env, cfg.policy ,routes, scenario_file, single_route, args.host, port)
            env.seed(seed)
            return env
        return _init
    ...
    #model = TD3("CnnPolicy", envs, verbose=1, buffer_size=38400,train_freq=100, policy_kwargs=policy_kwargs, tensorboard_log="./ppo_tensorboard/")
    #model = TD3("CnnPolicy", envs, verbose=1, buffer_size=38400, policy_kwargs=policy_kwargs, tensorboard_log="./ppo_tensorboard/")
