...
class CustomCNN(BaseFeaturesExtractor):
    ...
    def forward(self, observations: torch.Tensor) -> torch.Tensor:
        ...
        with torch.no_grad():
            ...
            observations[abs(observations-0.863)<=0.01]=1
            ...
            self.observation = observations[0,1]
            #print(self.observation.max())
            #print(self.observation.min())
            
            
            #or (observations[:,1:2]-0.863)<0.1
            ...
            self.human_map = rgb_image[:,1:2]
            #import matplotlib.pyplot as plt
            #plt.imshow(self.human_map[0,0].cpu())
...
def main(args, cfg, seed=6):
    ...
    # Optionally, save the trained model
    file_name = "seg_guided_carla_model_{}_{}".format(current_time,scenario_file[:-18:-12])
    model.save(file_name)
