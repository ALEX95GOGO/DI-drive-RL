...
from core.envs import DriveEnvWrapper, ScenarioCarlaEnvCamEval
...
class CustomCNN(BaseFeaturesExtractor):
    ...
    def __init__(self, observation_space: spaces.Box, features_dim: int = 256):
        ...
        #self.cam_model = resnet18(pretrained=True)
        #self.cam_model.cuda()
        #self.cam_model.eval()
...
#        with torch.no_grad():
#            cam_model = resnet18(pretrained=True)
#            cam_model.cuda()
#            cam_model.eval()
#            ##model = Model().cuda().eval()
#            ##checkpoint = torch.load('human_model/model_epoch_6.tar')
#            #checkpoint = torch.load('/data/zhuzhuan/VR_driving_data/data/script/CDNN_scenario2/ckpts/cdnn/model_epoch_5.tar')
#            ##model.load_state_dict(checkpoint['state_dict'])
#        #self.cam_model = self.cam_model.cuda()#.eval()
#            #self.model = Model().cuda().eval()
#            #checkpoint = torch.load('/data/zhuzhuan/CDNN-traffic-saliency/ckpts/cdnn/model_epoch_26.tar')
#            #checkpoint = torch.load('/data/zhuzhuan/VR_driving_data/data/script/eye_TD3/algo/checkpoints/car_following/model/eye/actor1707651533.pkl')
#            #import pdb; pdb.set_trace()
...
#            #plt.imshow(rgb_image[0].cpu().permute(1, 2, 0))
#            #plt.show(block=False)
#            #plt.pause(5.01)
#            ##output = model(rgb_image)
#            cam = CAM(cam_model, 'layer4', 'fc')
#            #rgb_image = rgb_image[:, :, :, 100:200]
#            #rgb_image = F.interpolate(rgb_image, size=(200, 200), mode='bilinear').clone()
#            rgb_image2 = normalize(rgb_image, [0.485, 0.456, 0.406], [0.229, 0.224, 0.225])
#            out = cam_model(rgb_image2)
#            output1 = cam(class_idx=817)[0]
#            output2 = cam(class_idx=705)[0]
#            output = torch.max(output1, output2)
#            #plt.imshow(output[0].cpu())
#            #plt.show(block=False)
#            #plt.pause(5.01)
#            #r = overlay_mask(to_pil_image(rgb_image[0]), to_pil_image(output[0], mode='F'), alpha=0.5)
#            #plt.imshow(r)
#            #plt.show(block=False)
#            #plt.pause(0.01)
#            # [B, C, H, W]
...
#            observations = F.interpolate(observations, size=(100,600), mode='nearest').clone()           
#            #self.human_map = output

        #import pdb; pdb.set_trace()
        #x = inp.unsqueeze(0) if len(inp.shape)==3 else inp
        observations_seg = observations[:, :, :, :300]
        N, Cin, H, W = observations_seg.shape
        #import pdb; pdb.set_trace()
        #x = F.max_pool2d( self.conv1(x), 2)
        #x = F.max_pool2d( self.conv2(x), 2)
        x = self.cnn(observations_seg)
...
def main(args, cfg, seed=0):
    ...
    checkpoint_folder = 'cam_guided_0522'
    checkpoint_folder = 'cam_guided'
    checkpoint_folder = 'cam_final'
    ...
    carla_env = DriveEnvWrapper(ScenarioCarlaEnvCamEval(cfg.env, cfg.policy, configs, args.host, args.port))
    carla_env.seed(seed)
    set_pkg_seed(seed)
    ...
    model = PPO("CnnPolicy", env, verbose=1, policy_kwargs=policy_kwargs, tensorboard_log="./ppo_tensorboard/", device='cuda')
    #model = PPO("CustomPolicy", env, verbose=1, policy_kwargs=policy_kwargs, tensorboard_log="./ppo_tensorboard/", device='cuda')
    
    #import pdb; pdb.set_trace()
    model_template = PPO.load(os.path.join(checkpoint_folder,chekpoint_list[ii][:-4]),deterministic=True)
    #model_template = PPO.load('cam_carla_model_20240522-151013_n3.zip')
    
    
    parameters = model_template.get_parameters()
...
#            plt.pause(0.001)
        ...
        if dones:
            ...
            if i >= 20:
                output_file_path = 'eval_new/{}_{}.csv'.format(checkpoint_folder,keyword)
                ...
                if ii >=len(chekpoint_list):
                    env.close()
                    break
                model = PPO.load(os.path.join(checkpoint_folder,chekpoint_list[ii]),deterministic=True)
                #break
