...
from core.envs import DriveEnvWrapper, ScenarioCarlaEnv3, ScenarioCarlaEnvCam
...
from torchcam.methods import CAM, ISCAM, ScoreCAM, GradCAM
...
def get_cam(model, input_tensor, target_id=817):
...
class CustomCNN(BaseFeaturesExtractor):
    ...
    def forward(self, observations: torch.Tensor) -> torch.Tensor:
        ...
        cam = GradCAM(cam_model, 'layer4')#, 'fc')
        ...
        #rgb_image2.requires_grad_(True)
        out = cam_model(rgb_image2)
        output1 = cam(class_idx=817, scores=out)[0]
        output2 = cam(class_idx=705, scores=out)[0]
        ...
        #output = get_cam(cam_model, rgb_image)
        #r = show_cam_on_image(rgb_img, output[0, :], use_rgb=True)
        plt.imshow(r)
        plt.show(block=False)
        plt.pause(0.01)
            # [B, C, H, W]
...
def main(args, cfg, seed=6):
    ...
    def make_env(port):
        def _init():
            ...
            #env = ScenarioCarlaEnv(cfg.env, configs[0], args.host, port)
            env = ScenarioCarlaEnvCam(cfg.env, cfg.policy ,routes, scenario_file, single_route, args.host, port)
            env.seed(seed)
    ...
    # Optionally, save the trained model
    file_name = "human_guided_carla_model_{}_{}".format(current_time,scenario_file[:-18:-12])
    model.save(file_name)
